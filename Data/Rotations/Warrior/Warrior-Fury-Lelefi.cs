...
namespace FrozenNamespace
{
    public class WarriorFury : CombatRoutine
    {
        ...
        public override void Pulse()
        {
            ...
            {	
				if (WoW.CanCast("DragonRoar") && WoW.Talent(7) == 3)
				{
            ...
            if (WoW.HasTarget && WoW.TargetIsEnemy && WoW.RangeToTarget <=5 && WoW.IsInCombat && !WoW.PlayerIsChanneling && WoW.TargetIsVisible &&
				ConfigFile.ReadValue<bool>("Fury-Warrior-Lelefi", "AutoRegeneration") && WoW.PlayerHealthPercent <= 20) 
            {	
				...
				if (WoW.CanCast("Commanding Shout"))
                {
			...
			if (ConfigFile.ReadValue<bool>("Fury-Warrior-Lelefi", "T21Rotation") && !ConfigFile.ReadValue<bool>("Fury-Warrior-Lelefi", "T20Rotation"))
			{
				...
				if (combatRoutine.Type == RotationType.SingleTarget && WoW.TargetHealthPercent <= 20 && WoW.HasTarget && WoW.TargetIsEnemy && WoW.RangeToTarget <=5 &&
			...
			if (!ConfigFile.ReadValue<bool>("Fury-Warrior-Lelefi", "T21Rotation") && ConfigFile.ReadValue<bool>("Fury-Warrior-Lelefi", "T20Rotation"))
			{
				...
				{	
					...
					if (WoW.CanCast("Rampage") && Ragepoints >= 100 && WoW.PlayerHasBuff("Meat-Cleaver") || WoW.CanCast("Rampage") && WoW.PlayerHasBuff("Massacre") &&
					...
					if (WoW.CanCast("OdynsFury") && WoW.SpellCooldownTimeRemaining("Raging-Blow") > 0 && WoW.PlayerHasBuff("Enrage") && WoW.PlayerHasBuff("BattleCryBuff"))
					{
						WoW.CastSpell("OdynsFury");
					...
					if (WoW.CanCast("Bloodthirst") && Ragepoints < 90 && !WoW.PlayerHasBuff("Enrage") && WoW.PlayerHasBuff("Meat-Cleaver"))
					{
						WoW.CastSpell("Bloodthirst");
						return;
					}
					...
					WoW.RangeToTarget <=5 && WoW.IsInCombat && !WoW.PlayerIsChanneling && WoW.TargetIsVisible) 
				{
					...
					if (WoW.CanCast("Rampage") && Ragepoints >= 100 || WoW.CanCast("Rampage") && WoW.PlayerHasBuff("Massacre"))
					{
						WoW.CastSpell("Rampage");
						return;
					}
					...
					WoW.RangeToTarget <=5 && WoW.IsInCombat && !WoW.PlayerIsChanneling && WoW.TargetIsVisible && WoW.Talent(6) == 3)
				{
					...
					if (WoW.CanCast("OdynsFury") && WoW.SpellCooldownTimeRemaining("Raging-Blow") > 0 && WoW.PlayerHasBuff("Enrage"))
					{
						WoW.CastSpell("OdynsFury");
						Log.Write("OdynsFury", Color.Green);
						return;
