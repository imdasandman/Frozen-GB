...
namespace FrozenNamespace
{
    public class Guardian : CombatRoutine
    {
        ...
        {
            Log.Write("Guardian Rotation v1.0 by Inhade");
            Log.Write("Supports all talent choices. However, recommended talents are 2131321 and 2331321");
            Log.Write("Best talent and option choices:");
            Log.Write("Resto affinity and Galactic Guardian");
            Log.Write("Barkskin and Survival either manual or on CD key");
            Log.Write("All else is situational, but feel free to play around regardless");
            Log.Write("Note that CDs and utility abilities are best triggered by the user, as they tend to be reactive");
            Log.Write("Adjust how aggresive healing logic you want to be by changing HP Rate of change treshold in options");
            Log.Write("A lower threshold triggers Frenzied Regen more easily and hence more often");
            Log.Write("This also modulates how easily other CDs will be cast based on if you are taking damage or not");
            Log.Write("as well as the fail-safe to switch to bear if you are bearcatting");
            Log.Write("You can also adjust the delay before the rotation interupts current target");
            Log.Write("Add your own spell id's to be interrupted in addition to existing PvP ones");
            Log.Write("The rotation incorporates the function to automatically change targets in AOE");
            Log.Write("Tick TabAOE if you want to use it in AOE rotation");
            Log.Write("You are encouraged to create a /targetenemy [nodead] macro and bind it to the TabMacro spell in PM Spellbook");
            Log.Write("AOE rotation focuses on picking up and holding aggro both in utility and rotation priorities");
            Log.Write("This causes a dps loss, so if you want to DPS efficiently in AOE choose the Cleave rotation");
            Log.Write("You can choose to bearcat if you have the feral affinity for a 30% dps increase");
            Log.Write("Always be careful not to be taking damage as it can be lethal if you are in cat form");
            Log.Write("This talent is highly situational, always go with Resto affinity if you are unsure");
            Log.Write("Use keybinds for defensive CDs for DPS burst");
            Log.Write("If you chose to bearcat, DPS Burst will work as the trigger to start the bearcatting rotation");
            Log.Write("Each CD keypress will first use both your Survival Insticts, then Rage if you chose to, then Barkskin");
            Log.Write("Enabling the overlay will display information on top of WoW");
            Log.Write("Choose what you want to be shown ticking the respective boxes");
            Log.Write("You can also change to prioritize magic or physical mitigation using a keybind");
            Log.Write("Feel free to ask anything in #druid and tag me!");
        ...
        private async Task keypress()
        {
            if ((DPSMod != 0 && DetectKeyPress.GetKeyState(DPSMod) < 0 || DPSMod == 0) && DetectKeyPress.GetKeyState(DPSKey) < 0)
                ...
                {
                    DPSBurst = !DPSBurst;
                    Log.Write("DPS Burst " + (DPSBurst ? "Activated" : "Deactivated"));
            ...
            if ((MitMod != 0 && DetectKeyPress.GetKeyState(MitMod) < 0 || MitMod == 0) && DetectKeyPress.GetKeyState(MitKey) < 0)
                if (MitigationSwitch && MitigationSwitchTimer.ElapsedMilliseconds > 1000)
                {
                    ...
                    {
                        Prioritize = 1;
                        Log.Write("Prioritizing for Magic Damage - Mark of Ursol");
                        Log.Write("Mark");
                    ...
                    {
                        Prioritize = 0;
                        Log.Write("Prioritizing for Physical Damage - Ironfur");
                        Log.Write("Ironfur");
                        if (DisplayMitigation)
                        {
        ...
        private async Task AsyncPulse()
        {
            ...
            if (WoW.CanCast("Ironfur") && (WoW.Rage >= 45 || PoolRage && WoW.Rage >= 90 ||
                ...
                && (Prioritize == 0 &&  !WoW.PlayerHasBuff("Ironfur") ||
                    Prioritize == 1 )
...
[SpellBook.db]
Spell,22812,Barkskin,X
Spell,20484,Rebirth,X
Spell,197626,Starsurge,X
Spell,197628,Lunar Strike,X
Spell,8921,Moonfire,X
Spell,5176,Solar Wrath,X
Spell,93402,Sunfire,X
Spell,197625,Moonkin Form,X
Spell,5215,Prowl,X
Spell,22842,Frenzied Regeneration,X
Spell,210037,Growl,X
Spell,33917,Mangle,X
Spell,77758,Thrash,X
Spell,192081,Ironfur,X
Spell,5487,Bear Form,X
Spell,1822,Rake,X
Spell,5221,Shred,X
Spell,1079,Rip,X
Spell,22568,Ferocious Bite,X
Spell,768,Cat Form,X
Spell,80313,Pulverize,X
Spell,213771,Swipe,X
Spell,6807,Maul,X
Spell,106839,Skull Bash,X
Spell,61336,Survival Instincts,X
Spell,102558,Incarnation: Guardian of Ursoc,X
Spell,99,Incapacitating Roar,X
Spell,204066,Lunar Beam,X
Spell,200851,Rage of the Sleeper,X
Spell,155835,Bristling Fur,X
Spell,16979,Wild Charge,X
Aura,22812,Barkskin
